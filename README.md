...
Плагин для [OpenCode](https://opencode.ai), который превращает встроенный `lsp` tool в семантически понятный для LLM интерфейс. Вместо указания точных координат (строка, колонка) модель передаёт имя символа и фрагмент кода, а плагин автоматически определяет позицию. Если LLM при этом передает некорректное имя фрагмента или недостаточно уникальный фрагмент кода, плагин к ответу от LSP подмешивает инструкции, как сделать вызов точнее, это снижает вероятность зацикливания вызова инструмента.
